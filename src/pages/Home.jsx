          ...
          <div className="hero-content">
            ...
            {hero.vote_average ? (
              <div className="hero-meta">
                <span className="hero-rating">IMDb {hero.vote_average.toFixed(1)}</span>
                {hero.vote_count ? (
                  <span className="hero-votes">{hero.vote_count.toLocaleString()} ratings</span>
                ) : null}
              </div>
            ) : null}
              ...
              {section.items.map((item) => (
                <article key={item.id} className="poster-card">
                  {item.poster_path ? (
                    ...
                    <div className="poster-placeholder">{item.title || item.name}</div>
                  )}
                </article>
              ))}
            </div>
